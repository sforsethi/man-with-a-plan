...
const whatsappNumber = '917011914397';
...
questionnaireForm?.addEventListener('submit', (event) => {
  ...
  const whatsappMessage = [
    '*MWP CLIENT QUESTIONNAIRE*',
    '_Let’s create something worth remembering._',
    '',
    `*01 — What should we call you?*\n${names}`,
    '',
    `*02 — Who are we creating this for?*\n${answer('creating_for')}`,
    '',
    `*03 — How can we reach you?*\nPhone: ${answer('phone')}\nEmail: ${answer('email')}`,
    '',
    `*04 — Where are you based?*\n${answer('based_in')}`,
    '',
    `*05 — What are you planning?*\n${answer('planning')}`,
    '',
    `*06 — What stage are you at right now?*\n${answer('stage')}`,
    '',
    `*07 — How many people are we creating this experience for?*\n${answer('guest_count')}`,
    '',
    `*08 — Where do you see it happening?*\n${answer('destination')}`,
    '',
    `*09 — Do you already have a venue?*\n${answer('venue_status')}\nVenue: ${answer('venue_name')}`,
    '',
